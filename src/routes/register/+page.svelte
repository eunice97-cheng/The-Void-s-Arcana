<script>
  ...
  async function handleRegister() {
    if (!email || !password || !confirm) { error = 'Please fill in all fields.'; return; }
    ...
    // If Supabase email confirmation is disabled, the user is immediately signed in
    if (data.session) {
      goto('/prelude');
    } else {
  ...
  <div class="card">
    {#if success}
      <div class="success-box">
        ...
        <p class="success-body">Check your email for a confirmation link, then <a href="/login">sign in</a> to begin your journey.</p>
      </div>
    {:else}
      <div class="field">
      ...
      <div class="field">
        ...
        <input id="password" type="password" bind:value={password} placeholder="At least 6 characters" on:keydown={handleKeydown} disabled={loading} />
      </div>
      <div class="field">
        <label for="confirm">Confirm Password</label>
        <input id="confirm" type="password" bind:value={confirm} placeholder="••••••••" on:keydown={handleKeydown} disabled={loading} />
      </div>
