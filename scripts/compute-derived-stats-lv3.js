// scripts/compute-derived-stats-lv3.js
...
const STAT_MULTIPLIERS = {
	...
	DEX: {
		warrior: { range: 1, acc: 0.0025, cr: 0.002, eva: 0.0015, atkSpd: 0.012 },
		archer: {
			range: 5,
			acc: 0.005,
			cr: 0.0025,
		...
		mage: { range: 1, acc: 0.0025, cr: 0.002, eva: 0.0015, healing: 0.0015, atkSpd: 0.015 },
		acolyte: { range: 1, acc: 0.0025, cr: 0.002, eva: 0.0015, healing: 0.0015, atkSpd: 0.015 },
		tinkerer: { range: 1, acc: 0.0025, cr: 0.002, eva: 0.0015, healing: 0.002, atkSpd: 0.01 }
	},
	INT: {
...
for (const cls of classes) {
	...
	const slotArrays = possibleSlots.map((slot) => {
		...
		arr.unshift({ id: 'none', name: 'none', slot: slot });
		return arr;
	});
		...
		for (const w2 of weapons) {
			...
			for (const armorChoice of armorCombos) {
				...
				if (cls.id === 'warrior') statBonus = stats.STR * STAT_MULTIPLIERS.STR.warrior.pAtk;
					...
					statBonus = stats.INT * STAT_MULTIPLIERS.INT.mage?.mAtk || stats.INT * 5;
	...
	const str = stats.STR,
		...
		con = stats.CON,
		wis = stats.WIS;
	const maxHp = Math.max(1, Math.floor(100 + (c.startHp || 0) + con * conMult.hp));
	const maxSp = Math.max(1, Math.floor(100 + (c.startSp || 0) + wis * wisMult.sp));
	// base pDef/mRes from STR/INT
	let pDef = str * strMult.pDef;
	let mRes = int * intMult.mRes;
	// apply equipment defenses from armor pieces
